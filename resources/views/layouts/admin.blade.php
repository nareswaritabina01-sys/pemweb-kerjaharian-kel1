...
<body class="bg-[#f8fafc] text-gray-800 font-sans antialiased">
    ...
    <nav
        ...
        <div class="flex items-center space-x-2">
            ...
            <span class="ml-1 text-[10px] font-bold bg-teal-50 text-primary px-2 py-0.5 rounded-full">Admin</span>
        </div>
        <div class="flex items-center space-x-2 border-l pl-4 border-gray-200">
            <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->nama ?? 'Admin') }}&background=007A87&color=fff"
                class="w-8 h-8 rounded-full" alt="Avatar">
            <span class="text-gray-700 hidden md:inline text-sm">Halo, {{ auth()->user()->nama ?? 'Admin' }}</span>
        </div>
    </nav>
