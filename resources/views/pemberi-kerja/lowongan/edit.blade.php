...
@section('content')
    <div class="max-w-3xl mx-auto">
        <div class="mb-6">
            ...
            <p class="text-gray-600 text-sm">Perbarui detail pekerjaan yang Anda tawarkan.</p>
        </div>

        <form action="{{ route('pemberi-kerja.lowongan.update', $lowongan->id) }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="judul" value="{{ old('judul', $lowongan->judul) }}"
                    placeholder="Contoh: Tukang Kebun Harian"
            ...
            <div>
                ...
                <input type="text" name="nama_perusahaan"
                    value="{{ old('nama_perusahaan', $lowongan->nama_perusahaan) }}" placeholder="Kosongkan jika perorangan"
                    class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="kategori_id"
                        ...
                        @foreach ($kategoriList as $kat)
                            <option value="{{ $kat->id }}"
                                {{ (int) old('kategori_id', $lowongan->kategori_id) === $kat->id ? 'selected' : '' }}>
                                {{ $kat->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="kuota_pekerja" min="1"
                        value="{{ old('kuota_pekerja', $lowongan->kuota_pekerja) }}"
                        class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                    @error('kuota_pekerja')
                        <p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>
                    @enderror
                    <p class="text-[11px] text-gray-400 mt-1">Pelamar diterima saat ini:
                        {{ $lowongan->kuota_pekerja - $lowongan->sisa_kuota }} orang.</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="upah" min="0" value="{{ old('upah', (int) $lowongan->upah) }}"
                        placeholder="100000"
                        class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                    @error('upah')
                        <p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="satuan_upah"
                        class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary">
                        <option value="harian"
                            {{ old('satuan_upah', $lowongan->satuan_upah) === 'harian' ? 'selected' : '' }}>Harian</option>
                        <option value="borongan"
                            {{ old('satuan_upah', $lowongan->satuan_upah) === 'borongan' ? 'selected' : '' }}>Borongan
                        </option>
                    </select>
                    @error('satuan_upah')
                        <p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <textarea name="deskripsi" rows="4" placeholder="Jelaskan detail pekerjaan, jam kerja, dan kebutuhan lainnya"
                    class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary">{{ old('deskripsi', $lowongan->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="lokasi" id="input-lokasi-teks" value="{{ old('lokasi', $lowongan->lokasi) }}"
            ...
            <div>
                ...
                <p class="text-[11px] text-gray-400 mb-2">Klik pada peta atau geser marker untuk mengubah titik lokasi
                    pekerjaan.</p>
                ...
                <input type="hidden" name="latitude" id="input-latitude"
                    value="{{ old('latitude', $lowongan->latitude) }}">
                <input type="hidden" name="longitude" id="input-longitude"
                    value="{{ old('longitude', $lowongan->longitude) }}">
                @error('latitude')
                    <p class="text-red-500 text-[11px] mt-1">{{ $message }}</p>
                @enderror
                @error('longitude')
    ...
    @push('scripts')
        ...
        <script>
            const lokasiAwal = {
                lat: {{ old('latitude', $lowongan->latitude) }},
                lng: {{ old('longitude', $lowongan->longitude) }}
            };

            const peta = L.map('peta-pilih-lokasi').setView([lokasiAwal.lat, lokasiAwal.lng], 14);
